...
export function WatchList({stockData}) {
    ...
    const add_ticker = () => {
        ...
        if (newTickerInput && AVAILABLE_TICKERS.has(newTickerInput) && !tickers.includes(newTickerInput)) {
            updated_tickers = [...tickers, newTickerInput].sort();
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed' }}>
                        <thead>
                            <tr>
                                ...
                                <th style={{ width: '10%', borderBottom: '1px solid #ccc', padding: '8px', textAlign: 'left' }}>+ 2σ</th>
                                <th style={{ width: '15%', borderBottom: '1px solid #ccc', padding: '8px', textAlign: 'left' }}>Time</th>
                                <th style={{ width: '15%', borderBottom: '1px solid #ccc', padding: '8px', textAlign: 'left' }}>Actions</th>
                            </tr>
                        </thead>
                        <tbody>
                            {tickers.map((ticker_name, index) => (
                                <WatchListItem ticker_data={stockData[ticker_name] || null} removeTicker = {remove_ticker} index = {index} />
                            ))}
                        </tbody>
                    </table>
                )
